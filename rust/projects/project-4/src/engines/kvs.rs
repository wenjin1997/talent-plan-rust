...
impl KvStoreReader {
    ...
    /// 
    /// `safe_point` is updated to the last compaction gen after a compaction finishes.
...
impl KvStoreWriter {
    ...
    fn compact(&mut self) -> Result<()> {
        ...
        for entry in self.index.iter() {
            ...
            self.index
                .insert(entry.key().clone(), (compaction_gen, new_pos..new_pos + len).into());
            new_pos += len;
        }
...
/// Create a new log file with given generation number and add the reader to the readers
/// map.
/// 
